...
import com.vsulimov.memos.util.ScreenMiddleware.addScreenMiddlewares
import com.vsulimov.memos.util.ScreenMiddleware.removeScreenMiddlewares
...
abstract class AbstractScreenFragment(
     ...
     * - Adding screen-specific middlewares from [ScreenMiddlewaresProvider.provideScreenMiddlewares]
     *   if the fragment implements [ScreenMiddlewaresProvider] and is not being restored from a saved state.
     *
    ...
    override fun onViewCreated(
        ...
        if (this is ScreenMiddlewaresProvider && savedInstanceState == null) {
            addScreenMiddlewares(tag = provideTag(), middlewares = provideScreenMiddlewares())
        }
        findViewsById(view)
        applyWindowInsets()
        setOnClickListeners()
     ...
     * - Removing screen-specific middlewares added in [onViewCreated] if the fragment implements
     *   [ScreenMiddlewaresProvider] and is being removed, to prevent memory leaks.
     */
    override fun onDestroyView() {
        super.onDestroyView()
        if (this is ScreenMiddlewaresProvider && isRemoving) {
            removeScreenMiddlewares(tag = provideTag())
        }
    }
     ...
     */
    internal fun applyDefaultInsets(
